...
import { Events, Reviews, DecodedToken, Booking } from "@/models/models";
...
function homeuser() {
	...
	const handleReviewClick = (bookingId: number) => {
		...
		if (booking && booking.payments.length > 0) {
			setReviewData({
				userId: userProfile.user_id, // Ensure this is correctly set from userProfile
				paymentId: booking.payments[0].payment_id, // Use the first valid payment ID
				reviewText: "",
				rating: 0,
				eventId: booking.event.event_id, // Ensure this matches your event data
			});
		...
		const fetchUserProfile = async () => {
			...
			if (!accessToken) {
				...
				try {
					...
					// console.log("coba decode user id dari fetch " + userId);
		...
		if (alreadyBooked) {
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					try {
						const decodedToken = jwtDecode<DecodedToken>(accessToken);
						// console.log("Keperluan decoded token untuk book : ", decodedToken);
						const userId = decodedToken.user;
						const eventId = event_id;
						// console.log("Keperluan userId token untuk book : " + userId);
						// console.log("Keperluan accessToken untuk book : " + accessToken);
						// console.log("Keperluan eventId untuk book : " + event_id);
	...
	async function deleteBookedEvent(booking_id: number) {
		...
		if (result.isConfirmed) {
			try {
				...
				// console.log("User ID from token that wants to delete: " + userId);
				// console.log("Booking ID to delete: " + booking_id);
				...
				if (response.status === 200) {
					...
					// Optionally refresh the booked events list or redirect the user
		...
		if (result.isConfirmed) {
			try {
				...
				if (response.status === 201) {
					...
					// console.log("This is paymentId yang diambil " + paymentId);
				...
				Swal.fire(
					"Error!",
					error.response?.data?.message ||
						"An error occurred while paying the event.",
					"error"
				);
			}
		}
	...
	const handleSubmitReview = async () => {
		...
		// Log review data before sending
		...
		try {
			...
			const err = error as Error;
			Swal.fire("Error! You can only review once", err.message || "Failed to submit review.", "error");
		}
	};
	function SheetDemo() {
		return (
										...
										>
											...
											<div className="flex-1 text-center">
												...
												<p className="text-black mb-4">
													Ticket Available: {event.ticket_available}
												</p>
												<p>
													Date:{" "}
												...
												<p
													className={`font-semibold text-black text-lg mt-2 mb-4 ${quickSand.className}`}
												>
													Price: Rp. {event.price.toLocaleString()}
												</p>
												<div
													className={`grid grid-cols-1 sm:grid-cols-2 gap-4 ${quickSand.className}`}
												></div>
											</div>
													...
													<>
														...
														{bookedEvent?.event_type === "Completed" && (
															<button
																...
																className="bg-purple-500 text-white py-2 px-4 rounded-lg hover:bg-purple-600 mt-2"
															>
																Write a Review
															</button>
														)}
